...
func _ready() -> void:
	...
	game_stats.score_changed.connect(func(new_score: int):
		if new_score > 10:
			yellow_enemy_spawn_timer.process_mode = Node.ProcessMode.PROCESS_MODE_INHERIT
		if new_score > 50:
			pink_enemy_spawn_timer.process_mode = Node.ProcessMode.PROCESS_MODE_INHERIT
	)


func handle_spawn(eneme_scene: PackedScene, timer: Timer, time_offset: float = 1.0) -> void:
	spawner_component.scene = eneme_scene
